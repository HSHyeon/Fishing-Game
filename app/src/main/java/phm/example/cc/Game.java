...
public class Game extends Activity {
    ...
    public void sOnPopupClick(View v){
        ...
        intent.putExtra("character",ss.result );
        intent.putExtra("coin",ss.coin);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(resultCode==RESULT_OK) // 액티비티가 정상적으로 종료되었을 경우
        {
            ...
            {
                ss.result=data.getIntExtra("res",0);
                ss.coin=data.getIntExtra("coin",coin);
    ...
    private class Listener implements SensorEventListener {
        ...
        public void onSensorChanged(SensorEvent event) {
            if(count==0) {if(event.sensor.getType() != Sensor.TYPE_GYROSCOPE)  return;}
            if(count==1) {if(event.sensor.getType() != Sensor.TYPE_ACCELEROMETER)  return;}

            switch (mDisplay.getRotation()) {
                case Surface.ROTATION_0:
                    if(count==0)ss.move1(event.values[DATA_X],
                            event.values[AXIS_Y]);
                    else ss.move2(-event.values[DATA_X],
                            event.values[AXIS_Y]);
                    break;
